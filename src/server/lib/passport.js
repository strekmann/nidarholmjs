...
function fetchUser(userId, callback) {
  return User.findById(userId, "name username profile_picture groups").exec(
    (err, user) => {
      if (err) {
        ...
        .exec((err, user) => {
          ...
          if (!user) {
            return done(null, false, {
              message: "Ukjent e-postadresse eller brukernavn",
            });
          }
          return user
      ...
      (req, _accessToken, _refreshToken, profile, done) => {
        ...
        User.findOne({ facebook_id: profile.id }, (err, user) => {
          if (!user) {
            return done(
              new Error(
                "Facebook-konto er ikke koblet mot Nidarholm-konto. Dette kan gjøres fra brukersiden etter at du har logget inn.",
              ),
            );
          }
          return done(err, user);
      ...
      (req, _accessToken, _refreshToken, profile, done) => {
        ...
        User.findOne({ google_id: profile.id }, (err, user) => {
          if (!user) {
            return done(
              new Error(
                "Google-konto er ikke koblet mot Nidarholm-konto. Dette kan gjøres fra brukersiden etter at du har logget inn.",
              ),
            );
          }
          return done(err, user);
      ...
      (req, _token, _tokenSecret, profile, done) => {
        ...
        User.findOne({ twitter_id: profile.id }, (err, user) => {
          if (!user) {
            req.flash(
              "error",
              `Twitter-konto er ikke koblet mot Nidarholm-konto.
