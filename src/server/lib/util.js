...
module.exports.daterange = function(start, end) {
  ...
  if (end) {
    ...
    return (
      '<time datetime="' + endm.format() + '">' + endm.format("lll") + "</time>"
    );
  }
};
...
module.exports.fetch_city = function(postcode, callback) {
  if (postcode && postcode.match(/\d{4}/)) {
    request
      .get(
        "http://adressesok.posten.no/api/v1/postal_codes.json?postal_code=" +
          postcode,
      )
      .end((error, result) => {
        if (error) {
            ...
            mkdirp(directory, (err) => {
              if (err) {
                ...
                const command =
                  "convert " +
                  filepath +
                  " -resize 220x220^ -gravity center -extent 220x220 -strip -auto-orient " +
                  thumbnail_path;
                exec(command, (err, stdout, stderr) => {
                  if (err) {
                    console.error(err, stderr);
...
module.exports.save_file = function save_file(
  tmp_path,
  prefix,
  do_delete = true,
) {
...
module.exports.insert_file = function insert_file(
  filename,
  hex,
  permissions,
  tags,
...
module.exports.upload_file = function(
  tmp_path,
  filename,
  user,
  param_options,
  ...
  save_file(tmp_path, options.prefix, options.do_delete).then(
    (hex, mimetype, size) => {
